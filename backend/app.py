...
# Chave Secreta
app.config['SECRET_KEY'] = os.environ.get('FLASK_SECRET_KEY', 'default_dev_secret_key_needs_to_be_strong_and_random')
...
if DATABASE_URL_PROD:
    ...
    print(f"--- Conectando ao banco de dados: {db_uri_to_use.split('@')[-1].split('/')[0]} ---")
else:
...
class ChamadoPagamento(db.Model):
    ...
    def to_dict(self, include_acompanhamentos=False, include_fornecedor=False):
        ...
        if include_acompanhamentos and self.acompanhamentos: data['acompanhamentos'] = sorted([a.to_dict() for a in self.acompanhamentos.all()], key=lambda x: x['data_entrada'], reverse=True) # Adicionado .all()
        if hasattr(self, 'owner') and self.owner: data['owner_username'] = self.owner.username
        return data
...
def obter_ou_criar_fornecedor(data_fornecedor, user_id_atual):
    ...
    if fornecedor_id: # ID explícito de um fornecedor existente (provavelmente de um select)
        forn = Fornecedor.query.filter_by(id=fornecedor_id, user_id=user_id_atual).first()
        if not forn: 
            # Se o ID não pertence ao usuário, pode ser um erro ou tentativa de usar ID de outro.
            # Para criação de chamados, o fornecedor deve ser do próprio usuário ou criado por ele.
            raise ValueError(f"Fornecedor com ID {fornecedor_id} não encontrado ou não pertence a você.")
        return forn.id
    elif nome_fornecedor_input: # Nome foi digitado, tentar encontrar ou criar
        ...
        if forn: 
            return forn.id
        else: # Fornecedor com este nome não existe para este usuário, criar novo
            if num_id_fornecedor_input and Fornecedor.query.filter_by(numero_identificacao_fornecedor=num_id_fornecedor_input, user_id=user_id_atual).first():
                raise ValueError(f"Um fornecedor com o N° de Identificação '{num_id_fornecedor_input}' já existe para você.")
            # Dupla checagem pelo nome (case-insensitive) antes de criar para garantir unicidade do nome para o usuário
            if Fornecedor.query.filter(Fornecedor.nome_fornecedor.ilike(nome_fornecedor_input), Fornecedor.user_id == user_id_atual).first():
                 raise ValueError(f"Um fornecedor com o nome '{nome_fornecedor_input}' já existe para você. Selecione-o da lista ou use um nome diferente.")
            ...
            db.session.commit() # Commit para persistir e obter o ID
            return novo_fornecedor.id
    return None # Fornecedor é opcional para ChamadoDiverso e não foi fornecido ID nem nome.
...
def handle_chamado_diverso_id(id):
    ...
    if request.method == 'PUT':
        ...
        if 'fornecedor_id' in data : # Permite definir/alterar/remover fornecedor
             if data['fornecedor_id']: # Se um ID foi fornecido e não é string vazia
                forn = Fornecedor.query.filter_by(id=data['fornecedor_id'], user_id=current_user.id).first() 
                if not forn: return jsonify({"message":f"Fornecedor ID {data['fornecedor_id']} não encontrado ou não pertence a você."}), 404
                chamado.fornecedor_id = int(data['fornecedor_id'])
